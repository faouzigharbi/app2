// Valide les exercices de la fiche.
...
if (process.env.CONTRE_EXEMPLES) {
  ...
  pousse("a du 1 decale", parQuestion(1, 0),
    c => { c.controle.claims[0][1] += " + 1"; });
  pousse("valeur absolue de a mal levee", parQuestion(1, 0),
    c => { c.etapes[2][1] = c.etapes[2][1].replace("= 4 - √15", "= √15 - 4"); });
  pousse("radical de b mal sorti", parQuestion(1, 1),
    c => { c.controle.env.b = c.controle.env.b.replace("√540", "√520"); });
  pousse("produit a×b decale", parQuestion(1, 2),
    c => { c.controle.claims[0][1] = "5"; });
  pousse("identite remarquable du 5 faussee", parQuestion(1, 4),
    c => { c.controle.claims[0][1] = c.controle.claims[0][1].replace("4√3", "2√3"); });
  pousse("e mal reduit", parQuestion(1, 5),
    c => { c.controle.env.e = c.controle.env.e.replace("√49", "√64"); });
  pousse("c pris avec le mauvais signe", parQuestion(1, 5),
    c => { c.controle.claims[1][1] = "-(2 + √3)"; });
  pousse("d au carre decale", parQuestion(1, 6),
    c => { c.controle.claims[0][1] = "13"; });
  pousse("signe de d inverse", parQuestion(1, 6),
    c => { c.controle.claims[1][1] = "2√3"; });
  pousse("d + 2c n est plus entier", parQuestion(1, 7),
    c => { c.controle.env.c = c.controle.env.c.replace("4√3", "5√3"); });
  pousse("E decale", parQuestion(1, 8),
    c => { c.controle.claims[0][1] += " + 1"; });
  pousse("F ne s annule plus", parQuestion(1, 9),
    c => { c.controle.env.F = c.controle.env.F.replace("b/4", "b/2"); });
  pousse("G decale", parQuestion(1, 10),
    c => { c.controle.claims[0][1] = "3"; });
  pousse("étape dupliquée", parQuestion(1, 0),
