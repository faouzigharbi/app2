...
(function (racine) {
  ...
  const forme = sh => S.ecrireForme(sh.cible, sh.u, sh.v, sh.w);
  ...
  const libres = (...shs) => {
    const v = new Set();
    shs.forEach(s => { if (s.u) v.add(s.u); if (s.v) v.add(s.v); if (s.w) v.add(s.w); });
  ...
  function comparerParSigne(o) {
    ...
    return {
      ...
      controle: { type: 'signe', libres: o.libres, defs: o.defs || {},
  ...
  function comparerParDifferenceCalculee(o) {
    ...
    return {
      ...
      controle: { type: 'signe', libres: o.libres, defs: o.defs || {}, lie: o.lie,
