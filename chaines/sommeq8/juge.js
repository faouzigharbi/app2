// LE JUGE de la fiche « المجموع و الفرق في ℚ » — partagé.
//
...
//
// Le tirage y est DÉTERMINISTE — un cycle fixe de valeurs, jamais le hasard.
// Sans cela une faute serait fausse pour l'un et vraie pour l'autre, et l'élève
// pourrait contester à bon droit.
(function (racine) {
  'use strict';
  ...
  // La SUITE déterministe. Elle doit être longue : certains énoncés n'ont de
  // sens que sous une contrainte (« a < b »), et le juge tire jusqu'à ce
  // qu'elle soit satisfaite. Un cycle court la rendrait insatisfiable — et
  // c'est arrivé. Deux pas premiers entre eux avec leurs modules donnent 41 × 9
  // couples distincts, largement de quoi tomber sur un cas permis.
  let curseur = 0;
  const rnd = () => {
    const i = curseur++;
    const n = ((i * 37) % 41) - 20;
...
// n'a que les variables qu'on lui a données : les autres, présentes dans les
  ...
  // Un tirage déterministe doit REPARTIR du même point pour chaque question,
  // sinon deux appels sur la même question ne donneraient pas les mêmes
  // environnements — et le juge cesserait d'être un juge.
  function envs(c) { curseur = 0; return environnements(c); }
