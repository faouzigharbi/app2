...
<main>
  <div id="barre" class="row">
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      ...
      <a class="badge" href="../ex${n}.html">⛓ سلسلة البرهان</a>
    </div>
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      <span class="niveaux">
...
(function(){
  ...
  const R = window.Reel, E = window.Erreurs;
  ...
  function rendre() {
    ...
    data.forEach((q, qi) => {
      ...
      s.innerHTML = '<b>السؤال ' + (qi + 1) + '</b> — ' +
        (q.fautes.length > 1 ? 'خطآن اثنان' : 'خطأ واحد');
      d.appendChild(s);
      ...
      consigne.textContent = q.fautes.length > 1
        ? 'إليك حلّ تلميذ. أين الخطآن؟ انقر على المرحلتين.'
        : 'إليك حلّ تلميذ. أين الخطأ؟ انقر على المرحلة.';
      ...
      boutons.appendChild(bV); boutons.appendChild(bH); boutons.appendChild(bR);
      ...
      bV.onclick = () => {
        ...
        let txt = 'وجدت ' + trouves.length + ' من ' + vrais.length;
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>أين الخطأ؟ — حصر و مجال</title>
<link rel="stylesheet" href="../style.css">
</head>
<body>
<header><h1>أين الخطأ؟ — حصر و مجال</h1>
...
${EXOS.map(n => `  <a href="err${n}.html" style="display:block;padding:14px;background:#fff;border:1.5px solid #e8ecf2;border-radius:10px;text-decoration:none;color:#2c3e50">🔎 ${F.entete(n)}${F.PROBLEMES[n].titre} <small style="color:#95a5a6">(${F.PROBLEMES[n].questions} أسئلة)</small></a>`).join('\n')}
</div>
</main>
</body>
</html>
...
for (const n of EXOS) {
  ...
  fs.writeFileSync(path.join(DOS, `err${n}.html`), page(n, titre));
  console.log(`erreurs/err${n}.html — ${titre}`);
}
