...
(function (racine) {
  ...
  // -------------------------------------------------------------------------
  // INTERVALLES — la couche propre à cette fiche.
  //
  // « حصر و مجال » ne parle pas que de nombres : elle parle d'ENSEMBLES, et une
  // réponse comme « I ∩ J = [-1/2 ; 3/2] » n'est pas une égalité de nombres. Le
  // reste du noyau ne saurait ni la lire ni la vérifier — d'où cette couche.
  //
  // Un intervalle est { inf, sup, oInf, oSup } : deux bornes, qui sont des
  // nombres exacts du noyau (les radicaux y passeraient sans rien changer), et
  // deux booléens qui disent si le crochet est ouvert. L'infini est `null` — et
  // une borne infinie est toujours ouverte, ce que le constructeur impose.
  // -------------------------------------------------------------------------
  const INF = /^[-−]\s*∞$/, PINF = /^\+?\s*∞$/;

  function intervalle(txt) {
    const m = /^\s*([[\]])\s*(.+?)\s*;\s*(.+?)\s*([[\]])\s*$/.exec(String(txt));
    if (!m) throw new Error('مجال غير مقروء « ' + txt + ' »');
    const inf = INF.test(m[2]) ? null : analyser(m[2], {});
    const sup = PINF.test(m[3]) ? null : analyser(m[3], {});
    if (inf && sup && sCmp(inf, sup) > 0) throw new Error('مجال مقلوب « ' + txt + ' »');
    return { inf, sup, oInf: inf === null || m[1] === ']',
                       oSup: sup === null || m[4] === '[' };
  }

  // x appartient-il à I ? Le cas d'égalité est le seul qui demande à regarder
  // le crochet — et c'est justement celui que l'élève oublie.
  function dansI(x, I) {
    if (I.inf !== null) {
      const c = sCmp(x, I.inf);
      if (c < 0 || (c === 0 && I.oInf)) return false;
    }
    if (I.sup !== null) {
      const c = sCmp(x, I.sup);
      if (c > 0 || (c === 0 && I.oSup)) return false;
    }
    return true;
  }

  // Intersection : la plus grande des bornes basses, la plus petite des hautes.
  // À borne égale, le crochet ouvert l'emporte — il exclut, et une exclusion
  // d'un seul côté suffit.
  function interI(A, B) {
    const bas = (a, oa, b, ob) => {
      if (a === null) return [b, ob];
      if (b === null) return [a, oa];
      const c = sCmp(a, b);
      return c > 0 ? [a, oa] : c < 0 ? [b, ob] : [a, oa || ob];
    };
    const haut = (a, oa, b, ob) => {
      if (a === null) return [b, ob];
      if (b === null) return [a, oa];
      const c = sCmp(a, b);
      return c < 0 ? [a, oa] : c > 0 ? [b, ob] : [a, oa || ob];
    };
    const [inf, oInf] = bas(A.inf, A.oInf, B.inf, B.oInf);
    const [sup, oSup] = haut(A.sup, A.oSup, B.sup, B.oSup);
    return { inf, sup, oInf, oSup };
  }

  // Réunion : l'inverse — mais elle n'a de sens comme INTERVALLE que si les
  // deux se touchent. Deux morceaux disjoints ne se recollent pas, et rendre
  // quand même un intervalle serait mentir : on lève.
  function unionI(A, B) {
    const vide = I => I.inf !== null && I.sup !== null
      && (sCmp(I.inf, I.sup) > 0
          || (sCmp(I.inf, I.sup) === 0 && (I.oInf || I.oSup)));
    if (vide(interI(A, B))) throw new Error('اتّحاد مجالين منفصلين');
    const bas = (a, oa, b, ob) => {
      if (a === null || b === null) return [null, true];
      const c = sCmp(a, b);
      return c < 0 ? [a, oa] : c > 0 ? [b, ob] : [a, oa && ob];
    };
    const haut = (a, oa, b, ob) => {
  ...
  const memeBorne = (a, b) => (a === null && b === null)
                           || (a !== null && b !== null && sEgaux(a, b));
  const egalI = (A, B) => memeBorne(A.inf, B.inf) && memeBorne(A.sup, B.sup)
                       && A.oInf === B.oInf && A.oSup === B.oSup;

  const iTxt = I => (I.oInf ? ']' : '[')
    + (I.inf === null ? '-∞' : sTxt(I.inf)) + ' ; '
    + (I.sup === null ? '+∞' : sTxt(I.sup)) + (I.oSup ? '[' : ']');

  // « I ∩ J », « I ∪ J », « I », ou un intervalle écrit en toutes lettres.
  // Les deux opérateurs ne se mélangent jamais dans cette fiche, on lit donc
  // de gauche à droite sans avoir à trancher de priorité.
  function ensemble(expr, noms) {
    const jetons = String(expr).split(/\s*([∩∪])\s*/);
    const lire = t => {
      const n = t.trim();
      if (noms && n in noms) return noms[n];
      return intervalle(n);
    };
    let v = lire(jetons[0]);
    for (let k = 1; k < jetons.length; k += 2) {
      const autre = lire(jetons[k + 1]);
      v = jetons[k] === '∩' ? interI(v, autre) : unionI(v, autre);
    }
    return v;
  }

  // Une relation d'ENSEMBLES : « I ∩ J = [-1/2 ; 3/2] ». Rend '' si elle tient,
  // le grief sinon, et null si ce n'en est pas une — même contrat que
  // verifierRelation, pour que le validateur puisse essayer l'une puis l'autre.
  function verifierEnsemble(texte, noms) {
    ...
    for (const p of parts) {
      const I = ensemble(p, noms);
      if (ref && !egalI(ref, I)) {
        return `« ${s.trim()} » فاسدة (${iTxt(ref)} و ${iTxt(I)})`;
      }
      ref = I;
    }
    return '';
  ...
  // Des rationnels PRIS DANS l'intervalle — c'est ainsi que les encadrements
  // sont éprouvés : « -7/2 ≤ A ≤ 1/10 » n'est pas contrôlé sur un x, mais sur
  // des dizaines, bornes comprises quand elles sont atteintes.
  function pointsDe(I, combien) {
    const out = [];
    ...
    for (let essai = 0; out.length < combien && essai < 40 * combien; essai++) {
      ...
      const x = S(rat(Math.round(t * q), q));
  ...
  //
  // Cette fiche ajoute au jeu de signes ceux des ensembles — ∈ ∩ ∪ ℝ, les
  // crochets, le point-virgule des bornes, ∞ — et ceux de la comparaison,
  // ≤ ≥ ≠ < >. Sans eux, « أوجد I ∩ J » laisserait « I ∩ J » couler de droite à
  // gauche et l'élève lirait l'intersection à l'envers.
  const ARABE = /[؀-ۿ]/;
  // Le « : » n'est PAS de la partie : cette fiche ne s'en sert pas comme signe
  // de division, mais comme ponctuation arabe — et happé dans un morceau isolé
  // en LTR, il se retrouverait à l'autre bout de la ligne.
  const SIGNES = '0-9A-Za-zπ+\\-*×÷/=^().,|√<>≤≥≠∈∩∪∞ℝ;{}\\[\\]';
  const RUN = new RegExp('[' + SIGNES + ']+(?:\\s+[' + SIGNES + ']+)*', 'g');
