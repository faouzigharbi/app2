...
(function (racine) {
  ...
  const REGLES = {
    ...
    // La NÉGATION est une affirmation comme une autre. Une page qui répond
    // « non, ils ne sont pas alignés » doit être contredite si elle se trompe —
    // sans cette règle, seule la réponse « oui » était vérifiée. On exige aussi
    // que les points soient distincts : trois points confondus sont alignés.
    'non-alignes': (P, [a, b, c]) => !memePoint(P[a], P[b]) && !memePoint(P[a], P[c])
                                  && !memePoint(P[b], P[c])
                                  && !alignes(P[a], P[b], P[c]),
