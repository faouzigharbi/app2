// Émet les 4 paires preuveN.js / preuveN.html + index.html.
// Recopie aussi moteur.js depuis serie1 : une seule implémentation, pas deux.
//   node _build.js .
const fs = require('fs');
const path = require('path');
const { PREUVES } = require('./generateurs.js');
...
<script src="generateurs.js"></script>
...
(function(){
  ...
  document.getElementById('btnNew').addEventListener('click', function() {
    data = window.Preuves.construire(window.exerciceNumero);
    currentQ = 0;
    score = 0;
    loadQuestion();
  });
...
fs.copyFileSync(path.join(__dirname, 'moteur.js'), path.join(OUT, 'moteur.js'));

const liens = [];
for (const n of Object.keys(PREUVES)) {
  const id = 'preuve' + n;
  const titre = 'سلسلة برهان ' + n + ' — ' + PREUVES[n].titre;
  fs.writeFileSync(path.join(OUT, id + '.js'),
    `// Tirage initial ; le bouton « أرقام جديدة » en refait un.\n`
    + `window.exerciceNumero = ${n};\n`
    + `window.exerciceData = Preuves.construire(${n});\n`);
  fs.writeFileSync(path.join(OUT, id + '.html'), page(id, n, titre));
  liens.push({ id, titre, n: require('./generateurs.js').PAR_PAGE });
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="stylesheet" href="style.css">
  <title>سلاسل البرهان — الأعداد الصحيحة الطبيعية</title>
</head>
<body>
<h3>🧩 سلاسل البرهان — التفكيك إلى عوامل أوّلية</h3>
<p style="text-align:center;color:#95a5a6;font-size:.9em;margin-bottom:16px">
  7 أساسي — ${liens.length} سلاسل برهان، ${total} تمارين في كل تحميل<br>
