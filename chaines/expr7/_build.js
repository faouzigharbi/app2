// Émet une page par TYPE de la fiche « العبارات الحرفية » (7 أساسي), plus l'index.
//   node _build.js [dossier]
//
// Une page = un type entier. Toutes ses questions sont posées EN MÊME TEMPS,
// chacune dans un volet d'accordéon : rater la première n'empêche plus de
// traiter les suivantes. Chaque volet a son propre « تحقق » et son propre
// « التصحيح ».
//
...
// reçoit les mêmes étapes dans l'ordre. Le parent peut donc corriger sans
// savoir refaire l'exercice.
const fs = require('fs');
...
require('./gens.js');
const EXOS = Object.keys(A.PROBLEMES).map(Number).sort((a, b) => a - b);
...
<head>
  ...
  <style>
    ...
    /* La figure : centrée, jamais plus large que sa carte, et conservée à
       l'impression — sans elle l'énoncé du périmètre n'a plus de sens. */
    .figure { display:block; margin:6px auto 10px; max-width:100%; height:auto }
    .fq .figure { margin:2mm auto 3mm; max-width:78mm }
    @media print {
...
<main>
  <div id="barre" class="row">
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      <span class="badge">✏️ العبارات الحرفية</span>
      <a class="badge" href="index.html">↩ الفهرس</a>
      <a class="badge" href="err${id}.html">🔎 أين الخطأ؟</a>
    </div>
    <div style="display:flex;gap:10px;flex-wrap:wrap">
...
(function(){
  ...
  document.getElementById('btnNew').addEventListener('click', function() {
    data = window.Expr.construire(window.exerciceNumero);
    construire();
    window.scrollTo(0, 0);
  });
...
['noyau.js', 'figure.js', 'expressions.js', 'gens.js'].forEach(f =>
...
for (const n of EXOS) {
  ...
  fs.writeFileSync(path.join(OUT, id + '.js'),
    ...
    + `window.exerciceData = Expr.construire(${n});\n`);
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="stylesheet" href="style.css">
  <title>العبارات الحرفية — 7 أساسي</title>
</head>
<body>
<h3>✏️ العبارات الحرفية — أنواع شاملة</h3>
<p style="text-align:center;color:#95a5a6;font-size:.9em;margin-bottom:16px">
  7 أساسي — ${liens.length} أنواع، ${total} سؤالا في كل تحميل<br>
  كل صفحة تعرض نوعا كاملا بكل أسئلته في آن واحد، و المولّد يختار نموذجا مختلفا في كل تحميل
</p>
