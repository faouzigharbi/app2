...
const REGLES = [
  ...
  ['coefficient 1 écrit', t => /(^|[\s(\[+\-])1\s*[a-zA-Zπ√]/.test(t)],
...
for (let t = 0; t < TIRAGES; t++) {
  for (const p of Object.keys(F.PROBLEMES).map(Number)) {
    for (const q of F.tirer(p)) {
      ...
      const c = q.controle;
      if (c.type === 'reduction') {
        const red = A.analyser(c.reduit);
        const aIrr = A.IRR.some(k => red[k] && red[k].n !== 0);
        if (!aIrr) note('forme réduite sans irrationnel : on n’est plus dans ℝ', 'ex' + p + ' — ' + c.reduit);
        if (!/[\[\]]/.test(c.brut)) note('aucune paire de crochets à supprimer', 'ex' + p + ' — ' + c.brut);
      }
